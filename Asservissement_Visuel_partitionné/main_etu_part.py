...
for i in range(duration):
    ...
    ''' Pc : World points expressed in the camera frame '''
    P01 = np.transpose(P0)
    Pc = Mc0 @ P01

    ''' pc : Image of World points in meters (x and y in rows)'''
    pc = np.zeros((4, 4))
    # print(Pc)
    for j in range(4):
    ...
    #attention, on utilise les coordonnées de pc
    Ls[0:2, 0:6] = matrix_interaction(pc[0, 0], pc[1, 0], Pc[2, 0])
    Ls[2:4, 0:6] = matrix_interaction(pc[0, 1], pc[1, 1], Pc[2, 1])
    Ls[4:6, 0:6] = matrix_interaction(pc[0, 2], pc[1, 2], Pc[2, 2])
    Ls[6:8, 0:6] = matrix_interaction(pc[0, 3], pc[1, 3], Pc[2, 3])
    ...
    '''Compléter '''
    ''' J_xy de l'article '''
    # Lxy = matrice  constitué des colonnes 1 2 4 et 5 de la matrice d'interataction
    ...
    #1ere colonne
    Lxy[:, 0] = J_interaction[:, 0]

    #2eme colonne
    Lxy[:, 1] = J_interaction[:, 1]

    #4eme colonne
    Lxy[:, 2] = J_interaction[:, 3]

    #5eme colonne
    Lxy[:, 3] = J_interaction[:, 4]

    ''' J_z de l'article '''
    # Lz = colonnes 3 et 6 de la matrice d'interataction
    #Lz de taille 8x2
    Lz = np.zeros((8, 2))

    #3eme colonne
    Lz[:, 0] = J_interaction[:, 2]
    Lz[:, 1] = J_interaction[:, 5]
    ...
    control_1 = np.zeros((2,1))
    control_1[0] = gamma_omega_z * (angle_des - angle) #wz
    control_1[1] = gamma_t_z * (area_des - area) #vz
    print("control1",control_1)
    

    
    '''control part 2: vx, vy, wx, wy'''
    # control_2 = ...
    control_2 =np.zeros((4,1))
    control_2 = 0.1*np.linalg.pinv(Lxy) @ ((s_des - s) - Lz @ np.array([control_1[1], control_1[0]]))


    ''''Modifier'''
    ''' control law including the robot '''
    T = np.zeros((6, 1)) #torseur cinématique
    T[0:2] = control_2[0:2]
    T[2] = control_1[1]
    T[3:5] = control_2[2:4]
    T[5] = control_1[0]
    ...
    control = 0.02*np.linalg.pinv(puma.jacobe(Q.transpose())) @ T
